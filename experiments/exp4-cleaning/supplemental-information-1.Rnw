...
\title{Supplemental Information 1: Data cleaning }
\date{April 2020}
...
figurefile <- file.path("/home/data/refined/reef/R/figures", "embryophyta")
@
...
p <- select_freq_count_plot( idx_fab, relative_to = 3193, mytitle="Fabaceae", verbose = FALSE, 
                             lf_quant = 0.35, rf_quant = 0.85, lc_quant = 0.5, rc_quant = 0.99,
                             top_left = -1.0, top_even = -2, top_right = 1,
                             stretch = 0.0,
                             size = 3)
p
  ggsave( filename = "fabaceae.png", path = figurefile, device = "png", dpi = 300)
...
We realized that we may have Illumina adapters in our reads, so we used our pipeline on the data after using {\tt TrimGalore}.
<<>>=
load("/home/data/refined/reef/temp_R/raw.tree.June.5.RData")
original.g <- tree.g <- tree
load("/home/data/refined/reef/R/raw.tree.april.15.RData")
original <- tree

all_tax <- union(tree$tax_id, tree.g$tax_id)

full <- data.frame()
for (i in 1:length(all_tax)) {
  t.idx <- which(tree$tax_id == all_tax[i])
  g.idx <- which(tree.g$tax_id == all_tax[i])
  if ((length(t.idx)>0) & (length(g.idx)>0)) {
    tmp <- tree[t.idx, ]; tmp$kind <- "both"
    tmp$"br_bel_g"<- tree[t.idx, "br_bel"]
    tmp$"br_may_g"<- tree.g[g.idx, "br_may"]
  } else if (length(t.idx) == 0) {
    tmp <- tree.g[g.idx, ]; tmp$kind <- "no_t"
    tmp$"br_bel_g"<- tmp$"br_bel";    tmp$"br_may_g"<- tmp$"br_may"
    tmp$br_bel <- 0; tmp$br_may <- 0
  } else if (length(g.idx) == 0) {
    tmp <- tree[t.idx,]; tmp$kind <- "no_g"
    tmp$"br_bel_g" <- 0; tmp$"br_may_g" <- 0
  }
  full <- rbind(full, tmp )
}
library(tidyverse)

n.both <- which(full$kind == "both") %>% length
n.t <- which(full$kind == "no_t") %>% length
n.g <- which(full$kind == "no_g") %>% length

bth <- full[which(full$kind == "both") , ]
bth <- arrange(bth, desc(abs((br_bel + br_may) - (br_bel_g + br_may_g))))

p <- ggplot(bth, aes(x=log(br_bel + br_may), y=log(br_bel_g + br_may_g))) +
      geom_point(shape=20)
p
@

First let's look at the disagreements between the two methods.
<<>>=
outliers <- c()
for (i in 1:nrow(bth)) {
  if (abs(log(bth[i, "br_bel"] + bth[i,"br_may"]) - 
          log(bth[i,"br_bel_g"] + bth[i,"br_may_g"])) > 2)
...
Now look at those sites in {\tt tree} but not in {\tt tree.g}.
<<>>=
nog <- full[which(full$kind == "no_g") , ]
nog <- arrange(nog, desc(abs((br_bel + br_may) - (br_bel_g + br_may_g))))

p <- ggplot(nog, aes(x=(br_bel + br_may), y=(br_bel_g + br_may_g))) +
      geom_point(shape=20)
p
head(nog, n=30)
@
The biggest differences are again in unknown sequences.


Lastly, we look at those sites in {\tt tree.g} but not in {\tt tree}.
<<>>=
not <- full[which(full$kind == "no_t") , ]
not <- arrange(not, desc(abs((br_bel + br_may) - (br_bel_g + br_may_g))))

p <- ggplot(not, aes(x=(br_bel + br_may), y=(br_bel_g + br_may_g))) +
      geom_point(shape=20)
p
head(not, n=30)
@
They are all very marginally identified species. Chrysochromulina parva is potentially
interesting.
